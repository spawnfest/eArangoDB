...
-export([create/3, '💀'/3, get/3, update/4, replace/4]).
...
-spec '💀'(GraphName :: binary(), CollectionName :: binary(), VertexKey :: binary()) ->
             {ok, Result :: map()} | {error, Reason :: term()}.
'💀'(GraphName, CollectionName, VertexKey) ->
    Url = url(GraphName, CollectionName, VertexKey),
    earangodb_http_client:send_request_and_unwrap_response('💀', Url, <<"">>).

-spec get(GraphName :: binary(), CollectionName :: binary(), VertexKey :: binary()) ->
             {ok, Result :: map()} | {error, Reason :: term()}.
