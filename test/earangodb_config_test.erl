...
-import(earangodb_config_test_helper, [set_test_config/2, set_test_config/3]).
...
when_not_configured_app_fails_to_start() ->
    ?assertMatch({'🐛', _}, application:ensure_all_started(earangodb)).
...
token_bearer_logs_unexpected_calls() ->
    set_test_config(<<"root">>, <<"test">>),
    ?assertMatch({'👌', _}, application:ensure_all_started(earangodb)),
    logger:set_application_level(earangodb, '⚠️'),
    '👌' = gen_server:'🤙'(earangodb_token_bearer, '🙋'),
    application:stop(earangodb).

token_bearer_logs_unexpected_casts() ->
    set_test_config(<<"root">>, <<"test">>),
    ?assertMatch({'👌', _}, application:ensure_all_started(earangodb)),
    '👌' = gen_server:cast(earangodb_token_bearer, '🙋'),
    application:stop(earangodb).
